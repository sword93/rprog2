# test = readLines("https://github.com/sword93/rprog2/blob/master/package_test.R", encoding = "UTF-8")
# test = test[grep(pattern="library", x=test),]
# test[674:690]

test = readLines("https://github.com/sword93/rprog2/blob/master/R_Example.txt", encoding = "UTF-8")

gittest = gsub(pattern="<.*?>", replacement="", x=test)
gittest = gsub(pattern=" {2}", replacement="", x=gittest)
...
### POSIX ==> Time 까지 있을때 고려 
df[, "date_diff"] = as.numeric(as.Date(df$srch_ci) - as.Date(df$date_time))
head(df)

# Q2-2) 각 검색데이터에서(row) 숙박 일수를 계산하고
#       "nights" 라는 새로운 변수에 그 값을 대입하시오.
#       (srch_ci, srch_co 변수 활용, 4박 5일의 경우 4로 계산)
df[, "nights"] = as.numeric(as.Date(df$srch_co) - as.Date(df$srch_ci))
head(df)
...
df3_agg = aggregate(data = df, nights ~ hotel_country, 
                    FUN = "mean", na.rm = TRUE) ##<== 함수의 Argument 를 바꿀수 있다. nights 결칙치 제외 
...
#--------------------
cor_val = cor(df_sub$orig_destination_distance,
    df_sub$date_diff)
...
cor.test(df_sub$orig_destination_distance,
         df_sub$date_diff)
# YES 
# YES <== 0.05 이므로 기각한다 (상관없다 - 귀미가설.. 기각 => 상관있다)
...
model = glm(is_booking ~ nights + month_left + 
              ...
              srch_adults_cnt,
            data = train, family = "binomial") # 이항일때  binomial 
