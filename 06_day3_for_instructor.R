...
#### _____ ● 공분산 예제 ####
set.seed(1234)
num_1 = sample(1:10, size = 20, replace = TRUE)
num_2 = sample(1:10, size = 20, replace = TRUE)
num_1
num_2
...
agricolae::duncan.test(dd, trt = "cut", console = TRUE)
agricolae::duncan.test(dd, trt = "clarity", console = TRUE)
...
aggregate(data = iris, Sepal.Length ~ Species,
          FUN = function(x){round(mean(x)^2, 2)})
...
library("dplyr")
df_join = left_join(x = df, y = df_class, 
                    by = c("bin" = "class"))
head(df_join, 3)
...
#### _____ ● 목표 ####
library("data.table")
df = fread("all/train.csv", data.table = FALSE, 
           select = c(1:20, 22))
set.seed(100)
df = df[sample(1:nrow(df), size = 1000000),]
df[, "date_time"] = substr(df$date_time, 1, 10)
rownames(df) = NULL
head(df)
fwrite(df, "hotel_booking_train_1m.csv")
